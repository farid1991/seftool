...
int rtz_arm_write(struct sp_port *port, uint32_t arm_addr, uint32_t arm_len, const uint8_t *data)
{
    ...
    while (arm_len > 0)
    {
        ...
        // Expect “OK>”
        if (wait_for_answer(port, "OK>", 0) != 0)
        {
...
int rtz_arm_flash(struct sp_port *port, const char *fw_name)
{
    ...
    printf("Address: 0x%08X, Length: %u kbytes, %u blocks\n",
           arm_addr,
           arm_len / 1024,
           (arm_len + 0xFFFF) >> 16);
...
int rtz_arm_send_bootloader(struct sp_port *port)
{
    printf("Sending arm-bootloader...");

    char loader_name[256] = "./loader/rtz/arm_bootloader.bin";
    ...
    send_byte(port, STCMD_14_ARMCMDWITHREPLY);
    send_byte(port, 'H');
    if (wait_for_answer(port, "Ready", 0) != 0)
        return -1;
