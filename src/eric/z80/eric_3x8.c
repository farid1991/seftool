...
int eric_3x8_init_serv_mode(struct sp_port *port, struct phone_info *phone)
{
	if (eric_init_cmd(port) != 0)
		return -1;
	if (eric_z80_send_loader(port, "3xx_bootstrap.bin") != 0)
		return -1;
	...
	if (eric_init_cmd(port) != 0)
		return -1;
	if (eric_z80_send_loader(port, "970416_1215_PRGCXC132039") != 0)
		return -1;
...
int eric_3x8_get_channel_ind(struct sp_port *port)
{
	printf("CHANNEL IND: ");
...
int eric_3x8_get_sm_capability(struct sp_port *port)
{
...
void eric_3x8_read_splock(struct sp_port *port, struct phone_info *phone)
{
...
void eric_3x8_read_phonelock(struct sp_port *port, struct phone_info *phone)
{
...
{
	if (eric_3x8_init_serv_mode(port, phone) != 0)
		return -1;
	...
	eric_3x8_get_channel_ind(port);

	eric_3x8_read_phonelock(port, phone);
	printf("ELCK : %s\n", phone->elock);

	eric_3x8_read_splock(port, phone);
	printf("NCK : %s\n", phone->nck);
	printf("NSCK: %s\n", phone->nsck);
	printf("CCK : %s\n", phone->cck);
