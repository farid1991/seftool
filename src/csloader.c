...
int csloader_read_gdfs_var(struct sp_port *port, uint8_t block, uint16_t unit,
                           uint8_t *outbuf, size_t outsize)
{
    ...
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_GDFS, CMD_GDFS_READVAR,
                                             gdfsvar, sizeof(gdfsvar), cmd_buf);
    if (cmd_len <= 0)
        return -1;
                            ...
                            uint8_t *data, uint32_t size)
{
    ...
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_GDFS, CMD_GDFS_WRITEVAR,
                                             gdfs_var, size + 7, cmd_buf);
    free(gdfs_var);
...
int csloader_read_gdfs(struct sp_port *port, struct phone_info *phone)
{
    ...
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_GDFS, CMD_GDFS_READALL,
                                             NULL, 0, cmd_buf);
    if (cmd_len <= 0)
        return -1;
...
int csloader_start_fsx_server(struct sp_port *port)
{
    ...
    uint8_t cmd_buf[8];
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_START,
                                             NULL, 0, cmd_buf);
    if (cmd_len <= 0)
        return -1;
...
int csloader_get_working_directory(struct sp_port *port, char *cwd, size_t max_len)
{
    ...
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_GETDIR,
                                             NULL, 0, cmd_buf);
    if (cmd_len <= 0)
        return -1;
    ...
    size_t cwd_len;
    ucs2_to_ascii(repl.data, repl.length, GDFS_DATA_OFFSET,
                  cwd, max_len, &cwd_len);
...
int csloader_change_directory(struct sp_port *port, const char *dirname)
{
    ...
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_CHDIR,
                                             data, size, cmd_buf);
    if (cmd_len <= 0)
        return -1;
...
int csloader_stat(struct sp_port *port, const char *filename, ose_stat_t *st)
{
    ...
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_STAT,
                                             data, size, cmd_buf);
    if (cmd_len <= 0)
        return -1;
...
int csloader_make_directory(struct sp_port *port, const char *dirname)
{
    ...
    uint8_t cmd_buf[0x100 + 7];
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_MKDIR,
                                             data, size, cmd_buf);
    if (cmd_len <= 0)
        return -1;
...
int csloader_delete_file(struct sp_port *port, const char *filename)
{
    ...
    uint8_t cmd_buf[1000];
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_RMFILE,
                                             data, size, cmd_buf);
    if (cmd_len <= 0)
        return -1;
...
int csloader_delete_directory(struct sp_port *port, const char *dirname)
{
    ...
    uint8_t cmd_buf[1000];
    int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_RMDIR,
                                             data, size, cmd_buf);
    if (cmd_len <= 0)
        return -1;
                      ...
                      size_t filesize, const uint8_t *filebuffer)
{
    ...
    while (offset < filesize)
    {
        ...
        int cmd_len = cmd_encode_csloader_packet(CMD_CS_FSX, CMD_FSX_PUTFILE,
                                                 data, size, cmd_buf);
        free(data);
                             ...
                             const char *dest_dir)
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        else
        {
            ...
            if (strlen(src_subdir) > 0)
                snprintf(local_filename, sizeof(local_filename), "%s/%s/%s", src_dir, src_subdir, entry);
            else
