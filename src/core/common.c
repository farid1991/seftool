...
// ---------- byte helper ----------
uint8_t get_byte(uint8_t *p) { return p[0]; }

void set_byte(uint8_t *p, uint8_t v) { p[0] = v; }
...
int parse_cxc_article_to_rest_name(struct phone_info *phone, const char *cxc_article)
{
	...
	if (version[0] && article[0]) {
		(void)snprintf(phone->rest_name, sizeof(phone->rest_name), "%.*s_%.*s",
		               (int)(sizeof(phone->rest_name) / 2 - 1), article,
		               (int)(sizeof(phone->rest_name) / 2 - 1), version);
	} else {
		(void)snprintf(phone->rest_name, sizeof(phone->rest_name), "%.*s", (int)(sizeof(phone->rest_name) - 1),
		               temp);
	}
...
int ucs2_to_ascii(const uint8_t *buf, size_t buflen, size_t pos, char *out, size_t outlen, size_t *consumed)
{
	size_t i = 0, o = 0;
	for (;; i++) {
		size_t b = pos + i * 2;
...
void randomize(void) { rndseed = (uint32_t)time(NULL); }
