...
int serial_close(struct sp_port *port) { return sp_close(port); }
...
{
	int written = sp_blocking_write(port, buf, len, 0);
	if (written < 0) {
...
int serial_wait_answer(struct sp_port *port, const char *expected, int timeout_ms, int skiperrors)
{
	uint8_t buf[3];
	size_t received = 0;

	while (received < 3) {
		int rcv_len = serial_wait_packet(port, buf + received, 3 - received, timeout_ms);
		if (rcv_len <= 0) {
	...
	if (memcmp(buf, expected, 3) != 0) {
		if (skiperrors == 1)
			return 0;
		fprintf(stderr, "[serial_wait_answer] Unexpected reply: %.*s (expected %s)\n", 3, buf, expected);
		return -1;
	}
