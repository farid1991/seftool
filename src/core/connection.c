...
int wait_for_handshake(struct sp_port *port, struct phone_info *phone)
{
	...
	while (1) {
		...
		if (r > 0) {
			if (c == 'Z') // Sony Ericsson reply with 'Z'
			{
				...
				phone->type = ERICSSON_MP;
				return SP_OK; // success
			}
			if (c == '>') // Ericsson 3X7 reply with '>>'
			{
				r = serial_read(port, &c, 1, TIMEOUT);
				if (c == '>') {
					printf("\nConnected\n");
					printf("\nDetected Ericsson 3X7\n");
					phone->type = ERICSSON_337;
					phone->chipset = PHONE_Z80;
					return SP_OK; // success
				}
			}
			if (c == '2') // Ericsson 3X8 reply with '2'
			{
				printf("\nConnected\n");
				printf("\nDetected Ericsson 3X8\n");
				phone->type = ERICSSON_338;
				phone->chipset = PHONE_Z80;
				return SP_OK; // success
			}
			...
			if (c == 'H') // Ericsson 868 reply with 'H'
			{
				printf("\nConnected\n");
				printf("\nDetected Ericsson 868/R190\n");
				phone->type = ERICSSON_868;
				phone->chipset = PHONE_Z80;
				return SP_OK; // success
			}
			if (c == 'F') // Ericsson 888 reply with 'F'
			{
				printf("\nConnected\n");
				printf("\nDetected Ericsson 888/R250\n");
				phone->type = ERICSSON_888;
				phone->chipset = PHONE_Z80;
				return SP_OK; // success
			}
			if (c == 'U') // Ericsson A1018 reply with 'U'
			{
				printf("\nConnected\n");
				printf("\nDetected Ericsson A1018\n");
				phone->type = ERICSSON_1018;
				phone->chipset = PHONE_AVR;
				return SP_OK; // success
			}
			if (c == 'V') // Ericsson T28 reply with 'V'
			{
				printf("\nConnected\n");
				printf("\nDetected Ericsson T28\n");
				phone->type = ERICSSON_T28;
				phone->chipset = PHONE_AVR;
				return SP_OK; // success
			}
			if (c == 'T') // Ericsson T1x reply with 'T'
			{
				printf("\nConnected\n");
				printf("\nDetected Ericsson A1018/T1X/R310\n");
				phone->type = ERICSSON_T1X;
				phone->chipset = PHONE_AVR;
				return SP_OK; // success
			}
			if (c == 0x60) // Ericsson T1x reply with 'T'
			{
				printf("\nConnected\n");
				printf("\nDetected New Ericsson A1018/T1X/R310\n");
				phone->type = ERICSSON_T1X;
				phone->chipset = PHONE_AVR;
				return SP_OK; // success
			}
...
int send_question_mark(struct sp_port *port, struct phone_info *phone)
{
	...
	if (phone->avr_ignore_print == 0) {
		printf("Chip ID: %04X%s, Platform: %s \n", phone->chip_id, phone->new_security ? " [RESPIN]" : "",
...
int connection_open(struct sp_port *port, struct phone_info *phone)
{
	if (serial_open(port) != 0)
		return -1;
	if (wait_for_handshake(port, phone) != 0)
		return -1;

	// before emp protocol no need for extra handshake
	...
	if (send_question_mark(port, phone) != 0)
		return -1;
	if (connection_get_type(port, phone) != 0)
		return -1;

	if (phone->protocol_major == EMP_PROTOCOL_2) {
		return connection_set_speed(port, phone);
	} else if (phone->protocol_major == EMP_PROTOCOL_3 && phone->protocol_minor == 1) {
		phone->qhldr_sent = 0;
		phone->break_cid29 = 0;
		phone->break_cid36 = 0;
...
int connection_close(struct sp_port *port) { return serial_close(port); }
