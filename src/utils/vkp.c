...
static int isthatchar(const char *addr, size_t size, uint32_t position, uint32_t *elementsize, const char *ch)
{
	if (position >= size)
		return 0;
...
static int deltaoffset(vkp_patch_t *v, const char *addr, uint32_t size, uint32_t position, uint32_t *elementsize)
{
...
static int patchstring(vkp_patch_t *v, const char *addr, size_t size, uint32_t position, uint32_t *elementsize)
{
...
int vkp_dovkp(vkp_patch_t *v, const char *addr, uint32_t size)
{
	...
	for (position = 0, linenum = 1; position < size; position += elementsize, linenum++) {
		if (eos(addr, size, position, &elementsize) || deltaoffset(v, addr, size, position, &elementsize) ||
		    patchstring(v, addr, size, position, &elementsize))
			continue;

		anyline(addr, size, position, &elementsize);
		snprintf(v->errorstring, sizeof(v->errorstring), "%.*s", (int)elementsize, addr + position);
		v->errorline = linenum;
		return linenum;
	}
	return 0;
...
void vkp_patch_free(vkp_patch_t *patch) { vkp_set_free(&patch->patch); }
...
size_t vkp_collect_unique_blocks(const vkp_patch_t *patch, size_t flashblocksize, uint32_t *blocks, size_t maxblocks)
{
	size_t count = 0;
