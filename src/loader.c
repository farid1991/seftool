...
int wait_e3_answer(struct sp_port *port, const char *expected, int timeout_ms)
{
    uint8_t buf[3];
    int got = 0;
    ...
    if (memcmp(buf, expected, 3) != 0)
    {
        if (skiperrors == 1)
            return 0;
        fprintf(stderr, "Unexpected reply: %.*s (expected %s)\n", 3, buf, expected);
        return -1;
    }
...
int loader_send_binary_cmd3e(struct sp_port *port, const char *loader_name)
{
    ...
    else if (hello_buf[0] == 0xFC && hello_buf[1] == 0xFF) // CMD3E DB2000
    {
        printf("DB2000 CMD3E\n");
        return 0;
    }
...
int loader_break_db2000_cid29(struct sp_port *port, const char *loader_name)
{
    ...
    if (serial_write(port, empty, sizeof(empty)) < 0)
        goto error;

    uint8_t cmd3c[4] = {0x3C, 0xE6, 0x00, 0x00};

    if (serial_write(port, cmd3c, sizeof(cmd3c)) < 0)
        goto error;
...
int loader_activate_payload(struct sp_port *port, struct phone_info *phone)
{
    if (loader_type == LDR_CHIPSELECT)
    {
        ...
        rcv_len = serial_wait_packet(port, resp, 8, 50 * TIMEOUT);
        if (rcv_len <= 0)
            return -1;
...
int loader_send_qhldr_noact(struct sp_port *port, const char *loader_name)
{
    FILE *fh = fopen(loader_name, "rb");
    if (!fh)
    {
    ...
    // if (is_babe(buffer, fsize) == 0)
    // {
    //     printf("This file is not BABE\n");
    //     return -1;
    // }
    ...
    if (serial_write(port, (uint8_t *)"QH00", 4) < 0)
        goto error;
    if (wait_e3_answer(port, "EsB", 3 * TIMEOUT) < 0)
        goto error;
    if (serial_write(port, qh00, qh_size) < 0)
        goto error;
    if (wait_e3_answer(port, "EhM", 3 * TIMEOUT) < 0)
        goto error;
    ...
    if (serial_write_chunks(port, qa00, qa_size, 0x800) < 0)
        goto error;
    if (wait_e3_answer(port, "EaT", 3 * TIMEOUT) < 0)
        goto error;
    if (wait_e3_answer(port, "EbS", 3 * TIMEOUT) < 0)
        goto error;
    ...
    if (serial_write_chunks(port, qd00, qd_size, 0x800) < 0)
        goto error;
    if (wait_e3_answer(port, "EdQ", 3 * TIMEOUT) < 0)
        goto error;

    if (skiperrors == 1) // anycid exploit
    {
        uint8_t byteleft;
        int rcv_len = serial_wait_packet(port, &byteleft, 2, 3 * TIMEOUT);
        if (rcv_len <= 0)
            goto error;

        printf("Run flashed loader\n");
    ...
    {
        fprintf(stderr, "Unexpected reply %X %X\n", hello_buf[0], hello_buf[1]);
        goto error;
    }
...
int loader_send_qhldr(struct sp_port *port, struct phone_info *phone, const char *loader_name)
{
    ...
    if (loader_type == LDR_PRODUCT_ID || phone->chip_id != PNX5230)
...
int loader_shutdown(struct sp_port *port)
{
    ...
    uint8_t resp[2];
    int rcvlen = serial_wait_packet(port, resp, sizeof(resp), 50 * TIMEOUT);
    if (rcvlen <= 0)
        return -1;
...
int loader_send_csloader_db2010(struct sp_port *port, struct phone_info *phone)
{
    if (phone->erom_cid == 36) // Both RED and BROWN
    {
        ...
        if (loader_break_db2010_cid36(port, phone) != 0)
            return -1;
        if (loader_send_binary(port, phone, DB2010_CSLOADER_BRN_CID49_V23) != 0)
            return -1;
        return 0;
    }
...
//         case 29: // CSLOADER not working yet
//             if (loader_send_qhldr(port, phone, DB2000_CERTLOADER_RED_CID00_R3L) != 0)
//                 return -1;
//             if (loader_send_binary_cmd3e(port, "./loader/2000_3e") != 0)
//                 return -1;
//             if (loader_break_db2000_cid29(port, "./loader/041214 0759 MATCXC1325712_PRODUCTION R2Z") != 0)
//                 return -1;
//             if (loader_activate_payload(port, phone) != 0)
//                 return -1;

//             return loader_send_binary(port, phone, "./loader/db2000_viola_csloader_r1e.bin");

//         case 36:
//             return loader_send_binary(port, phone, );
...
int loader_send_oflash_ldr_db2000(struct sp_port *port, struct phone_info *phone)
{
    if (phone->erom_cid == 36)
    {
        if (loader_send_qhldr(port, phone, DB2000_PILOADER_RED_CID00_R1F) != 0)
            return -1;
        return loader_break_db2000_cid36(port, phone);
    }
    ...
    default:
        printf("DB2000 CID:%d not supported\n", phone->erom_cid);
        return -1;
    }
}
...
int loader_send_oflash_ldr_db2010(struct sp_port *port, struct phone_info *phone)
{
    if (phone->erom_cid == 36) // Both RED and BROWN
    {
        ...
        return loader_break_db2010_cid36(port, phone);
    }
    ...
    if (phone->erom_color == BROWN)
    {
        ...
        case 49:
            if (loader_send_qhldr(port, phone, DB2010_PILOADER_BROWN_CID49_R1A002) != 0)
                return -1;
            return loader_send_binary(port, phone, DB2010_FLLOADER_RED_CID49_R2B_DEN_PO);
        // DB2012
        case 51:
...
int loader_send_oflash_ldr_db2020(struct sp_port *port, struct phone_info *phone)
{
    ...
    default:
        fprintf(stderr, "[DB2020] CID:%d not supported\n", phone->erom_cid);
        return -1;
    }
}
...
int loader_send_bflash_ldr(struct sp_port *port, struct phone_info *phone)
{
    ...
    case DB2000:
        if (phone->erom_cid == 29)
        {
            ...
            if (loader_send_binary_cmd3e(port, DB2000_BREAK) != 0)
                return -1;
            if (loader_break_db2000_cid29(port, DB2000_PRODUCTION_R2Z) != 0)
                return -1;
            return 0;
        }
    ...
    case DB2010_2:
        ...
        else if (phone->erom_cid == 49 && phone->erom_color == BROWN)
        {
            ...
            skip_cmd = 0;
            if (loader_send_binary(port, phone, DB2010_FLLOADER_RED_CID49_R2B_DEN_PO) != 0)
                return -1;
            return 0;
        }
    ...
    case PNX5230:
        ...
        if (loader_send_qhldr(port, phone, "./loader/pnx5230_prologue") != 0)
            return -1;
        if (loader_send_binary(port, phone, "./loader/PNX5230_PRODUCTION") != 0)
            return -1;
        return 0;
