...
uint8_t rtz_avr_get_speed_val(int speed)
{
	switch (speed) {
	case 921600:
...
int rtz_avr_setool_set_speed(struct sp_port *port, int baudrate)
{
...
void rtz_setool_turn_on_phone(struct sp_port *port)
{
	printf("\nTurning on phone\n\n");
...
int rtz_avr_setool_get_flash_info(struct sp_port *port)
{
	CFI cfi;
		...
		if (i == 'P') {
			...
			for (int r = 0; r < cfi.NumberOfEraseBlockRegions; r++) {
				...
				printf("          0x%08X - 0x%08X : %u blocks x %u bytes\n", blk->address,
				       blk->address + region_size - 1, blk->blocks, blk->blocksize);
...
static int rtz_avr_writeblock(struct sp_port *port, uint32_t avraddr, int avrlen, const uint8_t *from, int algo)
{
	uint8_t crc;
...
int rtz_avr_write(struct sp_port *port, uint32_t avraddr, uint32_t avrlen, const uint8_t *from)
{
	...
	while (avrlen > 0) {
		...
		while (fls < avrflashblocksnum &&
		       (avrflashblocks[fls].address + avrflashblocks[fls].blocksize * avrflashblocks[fls].blocks) <=
		               avraddr) {
			fls++;
		}
...
int rtz_avr_setool_flash_bin(struct sp_port *port, uint32_t avrbase, const char *firmware)
{
	if (!avrbase)
		avrbase = 0x800000;
...
int rtz_setool_rip_flash(struct sp_port *port, const char *fname, int what, uint32_t addr, uint32_t size)
{
	char dump_name[512];
	snprintf(dump_name, sizeof(dump_name), "./backup/flash_%08X_%s.bin", addr, fname);
	...
	while (size > 0) {
		...
		printf("\rReading addr 0x%08X (%u bytes)", addr, read_size);
