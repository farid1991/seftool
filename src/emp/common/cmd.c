...
int cmd_encode_write_gdfs(uint8_t block, uint8_t lsb, uint8_t msb, const uint8_t *data, int datasize, uint8_t *outbuf)
{
...
int cmd_encode_csloader_packet(uint8_t cmd, uint8_t subcmd, const uint8_t *csdata, int datasize, uint8_t *outbuf)
{
	int num = 0;
	uint8_t checksum = 0;
...
int cmd_decode_packet_ack(const uint8_t *buf, int size, struct packetdata_t *out)
{
	...
	if (buf[0] != SERIAL_ACK) {
		fprintf(stderr,
		        "[cmd_decode_packet_ack] Invalid reply\nExpected: 0x06 0x89: Got: 0x%02X "
		        "0x%02X\n",
		        buf[0], buf[1]);
		return -1;
	}
...
int cmd_decode_packet(const uint8_t *buf, int size, struct packetdata_t *out)
{
	...
	} else if (buf[0] == 0 && buf[1] == SERIAL_HDR89) // some CSLOADER or DB2000?
	{
		return cmd_decode_packet_noack(buf + 1, size - 1, out);
	           ...
	           buf[1] == SERIAL_HDR89) // DB2010_RESPIN_PRODLOADER_SETOOL2 & DB2020_PRELOADER_FOR_SETOOL2
	{
		return cmd_decode_packet_noack(buf + 1, size - 1, out);
	...
	} else if (buf[0] == SERIAL_ACK && buf[1] == SERIAL_ACK &&
	           buf[2] == SERIAL_HDR89) // patched cmd3E CID36 from den_po
	{
